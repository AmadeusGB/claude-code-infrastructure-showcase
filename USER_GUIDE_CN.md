...
代理通过 **Task 工具**调用。在 Claude Code 对话中，你只需要**描述任务**，Claude 会根据任务描述智能选择合适的代理并启动它。

**代理选择机制说明：**
- **与技能不同**：代理目前**没有**像 `skill-rules.json` 那样的配置文件来自动触发
- **智能判断**：Claude 根据你描述的任务内容和上下文，利用自然语言理解能力来选择最合适的代理
- **显式指定**：你也可以直接指定使用哪个代理，例如："使用 code-architecture-reviewer 审查我的代码"
...
|------|--------------|--------------|
| **自动选择机制** | ✅ **有** - 基于 `skill-rules.json` 配置 | ❌ **无** - 依靠 Claude 的智能判断 |
| **配置文件** | ✅ `skill-rules.json` - 关键词和意图模式匹配 | ❌ 没有配置文件 |
| **触发方式** | ✅ 通过 `UserPromptSubmit` 钩子自动触发 | ❌ 没有自动触发钩子 |
| **选择依据** | 规则匹配（关键词、意图模式、文件路径） | 自然语言理解（任务描述和上下文） |
| **配置示例** | ```json<br>{"keywords": ["backend", "API"],<br> "intentPatterns": ["(create\|add).*?route"]}<br>``` | 代理文件中的 `description` 字段 |
| **是否需要配置** | ✅ 需要配置 `skill-rules.json` | ❌ 无需配置，直接使用 |
| **使用场景** | 开发过程中的实时指导和最佳实践 | 复杂任务的自主执行和审查 |

**总结：**
- **技能**：有完整的自动激活机制，通过配置文件匹配触发
- **代理**：依靠 Claude 的自然语言理解能力来选择，更灵活但需要 Claude 理解任务意图

**如果你需要为代理创建类似的自动选择机制**，可以：
1. 创建一个 `agent-rules.json` 配置文件
2. 创建一个类似的钩子（如 `agent-activation-prompt.sh`）
3. 参考 `skill-activation-prompt.ts` 的实现模式

---

## 📊 完整对比：Skills、Agents、Commands

在深入了解斜杠命令之前，先看看这三者的完整区别：

| 特性 | 技能 (Skills) | 代理 (Agents) | 斜杠命令 (Commands) |
|------|--------------|--------------|-------------------|
| **定义** | 开发指南和最佳实践 | 自主执行的复杂任务处理器 | 结构化的工作流模板 |
| **触发方式** | ✅ 自动（通过 hooks + skill-rules.json） | 🤖 智能判断或显式指定 | 💬 用户输入斜杠命令（如 `/dev-docs`） |
| **配置文件** | ✅ `skill-rules.json` | ❌ 无 | ❌ 无（命令文件自带配置） |
| **执行方式** | 📚 提供指导和上下文 | 🔄 自主执行任务并返回报告 | 📝 执行结构化流程并生成文件 |
| **输出** | 代码建议和模式 | 完整的审查/修复报告 | 文档文件（plan.md, context.md, tasks.md） |
| **使用时机** | 开发过程中实时指导 | 任务完成后审查/复杂任务处理 | 开始复杂任务前规划 |
| **持久化** | ❌ 不生成文件 | ❌ 任务完成后消失 | ✅ 生成持久化文档 |
| **依赖关系** | 需要 hooks 支持 | 独立运行 | 独立运行 |
| **示例** | "创建后端路由" → 自动建议 `backend-dev-guidelines` | "审查我的代码" → 使用 `code-architecture-reviewer` | `/dev-docs 实现支付功能` → 生成规划文档 |
...
- ✅ 需要遵循项目最佳实践
- ✅ 创建新功能时确保符合规范
- 示例："帮我创建一个 Express 路由" → 自动激活 `backend-dev-guidelines`

**使用 Agents（代理）时：**
- ✅ 任务完成后需要审查
- ✅ 需要自主执行的复杂任务
- ✅ 需要专业分析和报告
- 示例："审查我刚写的 UserController" → 使用 `code-architecture-reviewer`

**使用 Commands（命令）时：**
- ✅ 开始复杂任务前需要规划
- ✅ 需要持久化的任务跟踪
- ✅ 需要跨会话继续的任务
- 示例：`/dev-docs 实现用户认证系统` → 生成规划文档

### Hooks 的作用

**Hooks 主要为 Skills 服务**，但也可以用于其他功能：

| Hook 类型 | 主要服务对象 | 功能 |
|-----------|------------|------|
| `UserPromptSubmit` | **Skills** | 分析用户输入，自动建议相关技能 |
| `PreToolUse` | Skills（可扩展） | 在工具执行前检查，可阻止不符合规范的操作 |
| `PostToolUse` | **Skills** + 文件跟踪 | 跟踪文件修改，维护上下文 |
| `Stop` | 构建检查（可选） | 在停止时执行检查（如 TypeScript 编译） |

**Hooks 的核心价值：**
- 🔧 **自动化**：让 Skills 自动激活，无需手动触发
- 📊 **上下文管理**：跟踪文件变化，维护项目状态
- 🛡️ **质量控制**：可阻止不符合规范的操作
