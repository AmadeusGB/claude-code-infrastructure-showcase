...
Always read @ai_context/IMPLEMENTATION_PHILOSOPHY.md (if available) and @ai_context/MODULAR_DESIGN_PHILOSOPHY.md (if available) (if available) first (if available in your project context).
...
- Consult @DISCOVERIES.md (if available) for similar patterns (if available)
- Document architectural decisions
- Check decision records in @ai_working/ (if available)decisions/ (if available)
