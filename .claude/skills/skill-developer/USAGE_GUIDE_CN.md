...
请帮我创建完整的技能结构，包括 SKILL.md、skill-rules.json 配置，并遵循 500 行规则。
```
...
请创建完整的技能，包括主文件和资源文件，遵循 500 行规则。
```
...
请使用 skill-developer 帮我创建这个技能，确保所有规则都正确集成。
```
...
- 关键词：route, endpoint, API, controller
...
请创建 vue-dev-guidelines 技能，包括完整的触发规则和内容。
```
...
1. **信息越详细越好** - skill-developer 需要足够的信息才能生成完善的技能
2. **明确技能类型** - 强制规范用 guardrail，指导建议用 domain
3. **提供测试场景** - 说明什么时候应该触发，什么时候不应该
4. **说明项目结构** - 帮助 skill-developer 创建准确的路径模式
5. **列举常见问题** - 帮助创建有针对性的内容
